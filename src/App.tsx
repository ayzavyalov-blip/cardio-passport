...
const RD = {
  ...
  validation: {
    ...
    total_protein: { min: 30,  max: 120,  label: "Общий белок (г/л)" },
  ...
  lab_ref: {
    // Холестерин: общепопуляционный порог — КР МЗ РФ «Нарушения липидного обмена» 2023; ESC/EAS Guidelines 2019
    cholesterol: { hi: 5.2,  unit: "ммоль/л", note: "< 5,2 ммоль/л — КР МЗ РФ «Нарушения липидного обмена» 2023" },
    // МК: норма у женщин 150–350 мкмоль/л (РИА, инпромед.ру); целевой ≤ 360 мкмоль/л — EULAR 2022
    uric_acid:   { hi: 350,  lo: 150, unit: "мкмоль/л", note: "Норма у женщин: 150–350 мкмоль/л; целевой (EULAR 2022): ≤ 360 мкмоль/л" },
    // Альбумин: норма взрослых 35–52 г/л — стандарт лабораторий РФ; нутритивный порог < 35 г/л — ESPEN 2021
    // Общий белок: норма 65–85 г/л (взрослые до 60 лет) — референтные значения лабораторий РФ; Меньшиков В.В. «Клиническая биохимия». Поле справочное, в скоринг не включается.
    total_protein: { lo: 65, hi: 85, unit: "г/л", note: "Норма: 65–85 г/л (референтные значения лаб. РФ). При < 65 г/л — дообследование: альбумин, функция печени и почек" },
    albumin:     { lo: 35,   hi: 52,  unit: "г/л", note: "Норма: 35–52 г/л (лаб. стандарт РФ). < 35 г/л — нутритивный дефицит (ESPEN 2021)" },
    // ТТГ: лаб. норма 0,4–4,0 мЕд/л; прегравидарный целевой 0,4–2,5 мЕд/л при АТ-ТПО(+) или терапии — МАРС 2024
    tsh:         { lo: 0.4,  hi: 2.5, unit: "мЕд/л", note: "Лаб. норма: 0,4–4,0 мЕд/л. Прегравидарный целевой (МАРС 2024 v3.1): 0,4–2,5 мЕд/л" },
    // Глюкоза: норма натощак < 6,1 ммоль/л (плазма, ВОЗ 1999); порог риска ГСД ≥ 5,1 ммоль/л — ВОЗ 2013; КР МЗ РФ «ГСД» 2020
    glucose:     { hi: 5.1,  unit: "ммоль/л", note: "Норма натощак: < 6,1 ммоль/л (ВОЗ). Порог риска ГСД: ≥ 5,1 ммоль/л (ВОЗ 2013; КР МЗ РФ «ГСД» 2020)" },
    // Ферритин: дефицит < 30 мкг/л (ВОЗ/WHO 2020); прегравидарный целевой ≥ 40 мкг/л — МАРС 2024 v3.1
    ferritin:    { lo: 30,   hi_opt: 40, unit: "мкг/л", note: "Дефицит: < 30 мкг/л (ВОЗ 2020). Целевой до зачатия: ≥ 40 мкг/л (МАРС 2024 v3.1)" },
  },
  red_flags: [
  ...
  recs: [
    ...
    },
    {
      ...
      cond:(d,m)=>(d.smoke_cig==='Да'||d.smoke_vape==='Да'||d.smoke_hookah==='Да'||d.smoke_snus==='Да'),
      actions:[
    ...
    },
    {
      ...
      cond:(d,m)=>(d.coc_type==='КОК'||d.coc_type==='Пластырь'||d.coc_type==='Кольцо')&&d.bp_measured==='Нет',
      actions:[
    ...
    {
      domain:"ГСД в анамнезе",
      problem:"ГСД в прошлой беременности — высокий риск повторного ГСД и СД 2 типа",
      cond:(d,m)=>d.gdm==='Да',
      actions:[
        ...
        "Риск повторного ГСД при следующей беременности: 30-50%; ранний ПГТТ на сроке 24-28 недель",
        "Снижение массы тела при ИМТ > 25: снижает риск повторного ГСД на 40-50%",
        "Физическая активность ≥ 150 мин/нед — независимо снижает риск нарушений углеводного обмена",
      ],
      pat:"ГСД в прошлый раз — это сигнал, что нужно особое внимание в следующей беременности. Проверьте сахар и следите за весом.",
    ...
    },
    {
      ...
      problem:"СКВ, АФС или АИТ — высокий риск осложнений беременности, разный профиль ведения",
      cond:(d,m)=>d.autoimmune && d.autoimmune!=='Нет' && d.autoimmune!=='',
      actions:[
        ...
        "При АИТ: контроль ТТГ и АТ-ТПО, при необходимости — коррекция дозы левотироксина до зачатия",
      ],
      pat:"При аутоиммунных заболеваниях беременность планируется в период ремиссии. Расскажите своему врачу (ревматологу при СКВ/АФС или эндокринологу при АИТ) о планах заранее — он скорректирует лечение.",
      src:"EULAR 2020; КР МЗ РФ «Заболевания щитовидной железы» 2021; Протокол МАРС 2024",
    },
    {
      domain:"Тромбоз в анамнезе",
      problem:"Тромбоз вен или артерий — высокий риск тромбоэмболических осложнений при беременности",
      cond:(d,m)=>d.thrombosis==='Да',
      actions:[
    ...
    // ── ЛАБОРАТОРНЫЕ ФАКТОРЫ ────────────────────────────────────────────────
    {
      ...
      cond:(d,m)=>{ const v = parseNum(d.cholesterol); return v !== null && v > 5.2; },
      actions:[
    ...
    },
    {
      ...
      problem:"Мочевая кислота > 350 мкмоль/л",
      cond:(d,m)=>{ const v = parseNum(d.uric_acid); return v !== null && v > 350; },
      actions:[
    ...
    },
    {
      ...
      cond:(d,m)=>{ const v = parseNum(d.albumin); return v !== null && v < 35; },
      actions:[
    ...
    },
    {
      ...
      cond:(d,m)=>{ const v = parseNum(d.glucose); return v !== null && v >= 5.1; },
      actions:[
    ...
    },
    {
      ...
      cond:(d,m)=>{ const v = parseNum(d.ferritin); return v !== null && v < 30; },
      actions:[
    ...
    },
    {
      ...
      cond:(d,m)=>{ const v = parseNum(d.ferritin); return v !== null && v >= 30 && v < 40; },
      actions:[
    ...
    },
    {
      ...
      cond:(d,m)=>{ const v = parseNum(d.tsh); return v !== null && (v < 0.4 || v > 2.5); },
      actions:[
    ...
    },
    {
      ...
      cond:(d,m)=>Number(d.miscarriages) >= 2,
      actions:[
    ...
    },
    {
      ...
      problem:"СД 2 типа у родителей, братьев или сестёр — наследственный риск нарушения углеводного обмена",
      cond:(d,m)=>d.family_dm==='Да',
      actions:[
        "Глюкоза натощак + HbA1c — оценка наследственного риска СД 2 типа до зачатия",
        "При нормальной глюкозе — повторный контроль раз в год, при планировании беременности — обязательно",
        "Поддержание ИМТ в норме (18,5–24,9) — снижает риск манифестации СД при наследственной предрасположенности",
        "Физическая активность ≥ 150 мин/нед и ограничение простых углеводов — первичная профилактика",
        "При выявлении преддиабета (глюкоза 5,6–6,9 ммоль/л натощак) — консультация эндокринолога до зачатия",
      ],
      pat:"Если у родителей или братьев/сестёр есть диабет, у вас повышен риск его развития. Сдайте анализ на сахар крови и следите за весом — это снижает риск.",
      src:"КР МЗ РФ «Сахарный диабет и беременность» 2023; КР МЗ РФ «ГСД» 2020",
    },
    {
      domain:"ССЗ в семейном анамнезе",
      problem:"Ранние ССЗ у родственников первой линии (мужчины < 55 лет, женщины < 65 лет)",
      cond:(d,m)=>d.family_cvd==='Да',
      actions:[
    ...
    // ── ОБРАЗ ЖИЗНИ ─────────────────────────────────────────────────────────
    {
      ...
      cond:(d,m)=>d.sleep==='Менее 7 часов',
      actions:[
    ...
    },
    {
      ...
      cond:(d,m)=>d.migraine==='С аурой'||d.migraine==='Возможная',
      actions:[
...
const DEFAULT_FORM = {
  ...
  cholesterol: '', uric_acid: '', total_protein: '', albumin: '',
...
const DEMO_FORM = {
  ...
  cholesterol: '4.8', uric_acid: '280', total_protein: '72', albumin: '42', tsh: '1.5', glucose: '4.9', ferritin: '45',
  labs_status: 'filled',
};
...
export default function App() {
  ...
  const metrics = useMemo(() => {
    ...
    // Если чекбокс «данных нет» установлен — 0%; иначе считаем реально заполненные
    const labKeys = LAB_KEYS;
    const filledLabs = form.labs_status === 'no_labs'
      ? []
      : labKeys.filter(k => { const v = parseNum(form[k]); return v !== null; });
    ...
    const clinicalFields: [string,string][] = [
      ...
      ['pe_own','ПЭ у самой'], ['gdm','ГСД'], ['chronic_htn','Хроническая АГ'],
      ['dm','Сахарный диабет'], ['ckd','Заболевание почек'],
      ['autoimmune','Аутоиммунные заболевания'], ['thrombosis','Тромбозы'],
    ];
    const missingClinical = [
      ...(!anySmokeFilled ? ["Курение (не указан ни один вид)"] : []),
      ...clinicalFields.filter(([k]) => k && !form[k]).map(([,l]) => l),
    ];
  ...
  const valid = useMemo(() => {
    ...
    // Общий белок — справочное поле, не включается в скоринг
    const tp = parseNum(form.total_protein);
    if (tp !== null) {
      if (tp < 65) labInt['total_protein'] = { text:'↓ Ниже нормы (< 65 г/л) — рекомендуется дообследование', color:'text-amber-600' };
      else if (tp > 85) labInt['total_protein'] = { text:'↑ Выше нормы (> 85 г/л) — рекомендуется консультация', color:'text-amber-600' };
      else labInt['total_protein'] = { text:'✓ В норме (65–85 г/л)', color:'text-green-600' };
    }
    // Шаг 3 (лаборатория) — необязательный, никогда не блокирует кнопку «Рассчитать»




    // Полнота базовых данных
    let filled = 0; const total = 16;
    if (parseNum(form.age)) filled++;
    if (form.patient_name?.trim().length > 1) filled++;
    if (form.family_pe) filled++;
    if (form.family_cvd) filled++;
    if (form.family_dm) filled++;
    if (form.spky) filled++;
    if (form.coc_type) filled++;           // исправлено: coc → coc_type
    if (form.migraine) filled++;
    ...
    if (completeness >= 75 && metrics.labsCompleteness === 100) reliability = 'Высокая';
    else if (completeness >= 75 && metrics.labsCompleteness > 0) reliability = 'Средняя+';
    else if (completeness >= 75) reliability = 'Средняя';
    else if (completeness >= 50) reliability = 'Низкая+';

    // Блокировки по шагам — согласие блокирует только шаг 1
    const step1Fields = ['consent_personal_data', 'patient_name'];
    const step2Fields = ['height','weight','waist','sys1','dia1','sys2','dia2','sys3','dia3'];
    const errorsForStep = (fields: string[]) => fields.some(k => fe[k]);
    // Шаг 3 — лаборатория необязательна, никогда не блокирует «Рассчитать»
    const currentStepInvalid = step===1 ? errorsForStep(step1Fields) : step===2 ? errorsForStep(step2Fields) : false;
  ...
  const scoring = useMemo(() => {
    ...
    if (form.autoimmune && form.autoimmune !== 'Нет') add('A',`Аутоиммунное заболевание (${form.autoimmune})`, 4);
    ...
    if (ua   !== null && ua   > 350)          add('H','Гиперурикемия (> 350 мкмоль/л у женщин)', 2);
  ...
  // ── ПРЕДВАРИТЕЛЬНАЯ ГЕНЕРАЦИЯ ЗАКЛЮЧЕНИЯ ────────────────────────────────────
  // Текст пересчитывается автоматически при изменении данных.
  // При нажатии «Рассчитать» — уже готов, переход мгновенный.
  const generateReport = useCallback(() => {
    const f = form; // объявляем f ДО первого использования
    const d = new Date().toLocaleDateString('ru-RU');
    const name = f.patient_name || 'Пациентка';
    const bmi = metrics.bmi;
    ...
    t += `Дата: ${d}\n`;
    t += `Пациентка: ${name}${f.age ? `, ${f.age} лет` : ''}\n`;
    if (bmi > 0) t += `ИМТ: ${bmi} кг/м²; АД среднее: ${bp}\n`;
    t += `\n`;

    t += `КАТЕГОРИЯ РИСКА: ${scoring.riskCat.toUpperCase()} (балл: ${scoring.total})\n\n`;
    ...
    const tacticsMap: Record<string,string> = {
      ...
      'Повышенный': 'Рекомендовано расширенное обследование. Консультации профильных специалистов до зачатия.',
    ...
    if (scoring.activeRF.length) {
      t += `ТРЕВОЖНЫЕ ПРИЗНАКИ:\n`;
      scoring.activeRF.forEach(rf => { t += `- ${rf.title} [${rf.urgency}]: ${rf.action}\n`; });
      t += `\n`;
    }

    if (scoring.top5.length) {
      t += `ВЕДУЩИЕ ФАКТОРЫ РИСКА:\n`;
      scoring.top5.forEach(fac => { t += `- ${fac.factor}\n`; });
      t += `\n`;
    } else {
      t += `Значимых факторов риска не выявлено. Показатели в пределах нормы.\n\n`;
    }
    ...
    if (scoring.activeRecs.length) {
      ...
      t += `РЕКОМЕНДАЦИИ:\n`;
      t += `- Фолиевая кислота 400 мкг/сут не менее чем за 3 месяца до зачатия\n`;
      t += `- Сбалансированное питание, физическая активность ≥ 150 мин/нед\n`;
      t += `- Отказ от алкоголя и курения\n`;
      t += `- Плановый визит к акушеру-гинекологу\n\n`;
    }

    if (metrics.missingLabs.length > 0)
      t += `Лабораторные данные не предоставлены: ${metrics.missingLabs.join(', ')}. Рекомендуется повторная оценка после получения результатов.\n\n`;
...
<span className="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest text-blue-300 border border-blue-700 bg-blue-900/50">ПГМУ им. Вагнера · Пермь</span>
  ...
  <p className={`text-sm font-black mb-3 ${form.consent_personal_data ? "text-green-800" : "text-amber-900"}`}>{form.consent_personal_data ? "✓ Согласие получено" : "Перед началом заполнения"}</p>
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 flex items-center justify-between gap-4">
                ...
                <StableSelect value={form.who_fills || role} onChange={v=>{ f({who_fills:v}); setRole(v); }} className="text-xs font-black border rounded-lg px-3 py-2 outline-none bg-white shrink-0">
              ...
              <div className="bg-white p-5 md:p-6 rounded-2xl border border-slate-200">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <StableSelect value={form.migraine} onChange={v=>f({migraine:v})} className={sel}>
                      <option value="">— Не указано</option>
                      <option value="Нет">Нет</option>
                  ...
                  <div>
                    ...
                    <div className="space-y-2 mt-1">
                      {[['smoke_cig','Сигареты / папиросы'],['smoke_vape','Вейп / электронные сигареты'],['smoke_hookah','Кальян'],['smoke_snus','Снюс / жевательный табак']].map(([key, label]) => (
                        <label key={key} className="flex items-center gap-2 cursor-pointer select-none">
                          <input type="checkbox" className="w-4 h-4 accent-blue-600"
                            checked={(form as any)[key] === 'Да'}
                            onChange={e=>f({[key]: e.target.checked ? 'Да' : 'Нет'})}/>
                          <span className="text-xs text-slate-700">{label}</span>
                        </label>
                      ))}
                      <p className="text-[10px] text-slate-400 mt-1.5">Если не курите — ничего не отмечайте</p>
                    </div>
                  </div>
                  <div>
                    <label className={lbl}>Алкоголь регулярно</label>
                    <StableSelect value={form.alcohol} onChange={v=>f({alcohol:v})} className={sel}><option value="">— Не указано</option><option>Нет</option><option>Да</option></StableSelect>
              ...
              <div className="bg-white p-5 md:p-6 rounded-2xl border border-slate-200">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <StableSelect value={form.calcium_daily} onChange={v=>f({calcium_daily:v})} className={sel}><option value="">— Не указано</option><option value="Да">Да, ежедневно</option><option value="Нет">Нет / редко</option></StableSelect>
                  ...
                  <div>
                    ...
                    <StableSelect value={form.diet_restrict} onChange={v=>f({diet_restrict:v})} className={sel}><option value="">— Не указано</option><option>Нет</option><option>Да</option></StableSelect>
              ...
              <div className="bg-white p-5 md:p-6 rounded-2xl border border-slate-200">
                ...
                <div className="mb-4">
                  ...
                  <StableSelect value={form.pregnant_ever} onChange={v=>f({pregnant_ever:v})} className={`${sel} max-w-xs`}>
                    <option value="">— Не указано</option>
                    <option value="Нет">Нет</option>
              ...
              <div className={`p-5 md:p-6 rounded-2xl border ${(form.chronic_htn==='Да'||form.dm==='Да'||form.ckd==='Да'||(form.autoimmune&&form.autoimmune!=='Нет')||form.thrombosis==='Да'||form.risky_meds==='Да') ? 'border-amber-200 bg-amber-50/30' : 'bg-white border-slate-200'}`}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div><label className={lbl}>Аутоиммунные заболевания</label>
                    <StableSelect value={form.autoimmune} onChange={v=>f({autoimmune:v})} className={sel}>
                      <option value="">— Не указано</option>
                      <option value="Нет">Нет</option>
                      <option value="СКВ">СКВ (системная красная волчанка)</option>
                      <option value="АФС">АФС (антифосфолипидный синдром)</option>
                      <option value="АИТ">АИТ (аутоиммунный тиреоидит)</option>
                      <option value="Другое">Другое аутоиммунное заболевание</option>
                    </StableSelect></div>
                  <div className="sm:col-span-2">
            ...
            <div className="space-y-6">
              <div className="bg-white p-5 md:p-8 rounded-2xl border border-slate-200 shadow-sm">
                ...
                <div className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 transition-opacity ${form.labs_status==='no_labs' ? 'opacity-30 pointer-events-none' : ''}`}>
                  {[
                    { k:'cholesterol', l:'Общий холестерин',
                      ref:'< 5,2 ммоль/л — КР МЗ РФ «Нарушения липидного обмена» 2023; ESC/EAS 2019',
                      unit:'ммоль/л' },
                    { k:'uric_acid',   l:'Мочевая кислота',
                      ref:'Норма у женщин: 150–350 мкмоль/л. Целевой (риск подагры): ≤ 360 мкмоль/л — EULAR 2022; КР МЗ РФ «Подагра» 2021',
                      unit:'мкмоль/л' },
                    { k:'total_protein', l:'Общий белок сыворотки',
                      ref:'Норма: 65–85 г/л у взрослых до 60 лет (референтные значения лабораторий РФ; Клиническая биохимия, Меньшиков В.В.). Поле справочное — в расчёт баллов не включается; при значении < 65 г/л рекомендуется дообследование (альбумин, функция печени и почек)',
                      unit:'г/л' },
                    { k:'albumin',     l:'Альбумин сыворотки',
                    ...
                    { k:'glucose',     l:'Глюкоза натощак',
                      ...
                      unit:'ммоль/л' },
                    { k:'ferritin',    l:'Ферритин',
                      ref:'Дефицит: < 30 мкг/л. Субоптимально: 30–39 мкг/л. Целевой до зачатия: ≥ 40 мкг/л — Протокол МАРС 2024 v3.1',
                      unit:'мкг/л' },
                  ].map(({k,l,ref,unit}) => {
              ...
              <div className="bg-white p-5 md:p-8 rounded-2xl border border-slate-200 shadow-sm space-y-5">
                ...
                {/* Незаполненные клинические поля */}
                {metrics.missingClinical.filter(Boolean).length > 0 && (
                  <div className="border-t border-slate-100 pt-4">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Не заполнено на шаге 1</p>
                    <div className="flex flex-wrap gap-2">
                      {metrics.missingClinical.filter(Boolean).map((field, i) => (
                        <button key={i}
                          onClick={() => { setStep(1); const el = document.querySelector('[data-scroll="top"]') as HTMLElement; if (el) el.scrollIntoView(); }}
                          className="text-[10px] font-bold px-2.5 py-1 bg-amber-50 border border-amber-200 text-amber-700 rounded-lg hover:bg-amber-100 transition flex items-center gap-1">
                          ← {field}
                        </button>
                      ))}
                    </div>
      ...
      <div className="bg-white border-t p-4 md:px-8 flex justify-between items-center shrink-0 shadow-lg z-20" style={{paddingBottom:'max(1rem, env(safe-area-inset-bottom))'}}>
        ...
        {step < 3 ? (
          ...
          <button
            disabled={valid.currentStepInvalid}
            ...
            onClick={() => {
              try {
                ...
                setReport(r);
                setHistory([{ date: new Date().toISOString(), text: r, author: 'Авточерновик' }]);
                setCalcHistory((prev) => [{
                ...
                // Всё равно переходим к результатам
                setView('results');
                setTab('summary');
              }
            }}>
            <FileCheck className="w-5 h-5"/> Рассчитать риск
          </button>
        )}
      </div>
    </div>
      ...
      return (
        <div className="overflow-y-auto h-full bg-slate-50">
          <div className="max-w-xl mx-auto p-4 space-y-4" id="print-area">
            ...
            {(metrics.bmi > 0 || metrics.avgSys > 0 || Number(form.waist) > 0) && (
              <div className="bg-white rounded-2xl border border-slate-200 p-5">
                  ...
                  {Number(form.waist) > 0 && (
                    <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50">
                      ...
                      <div className="text-right">
                        ...
                        <p className={`text-[10px] font-black ${Number(form.waist) >= 88 ? 'text-red-600' : Number(form.waist) >= 80 ? 'text-amber-600' : 'text-green-600'}`}>
                          {Number(form.waist) >= 88 ? 'Значительно выше нормы' : Number(form.waist) >= 80 ? 'Чуть выше нормы' : '✓ В норме'}
                        </p>
                      </div>
                    </div>
                  )}
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {scoring.activeRecs.map((r, i) => {
                      ...
                      const decoded = r.pat
                        .replace(/ГСД/g, 'гестационный диабет (диабет при беременности)')
